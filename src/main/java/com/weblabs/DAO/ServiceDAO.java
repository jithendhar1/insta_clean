...
public class ServiceDAO {
	 ...
	 public static List<ServiceBean> getFilteredServices(String whereClause, int start, int limit) {
	  ...
	  //from serviceID retrive
	  public static List<ServiceBean> getAllServices(String serviceID) {
		    List<ServiceBean> allS = new ArrayList<>();
		    Connection connection = null;
		    PreparedStatement preparedStatement = null;
		    ResultSet resultSet = null;
		    ...
		    try {
		        ...
		        // Modify the query based on the specified type
		        String query = "SELECT serviceID, servicename, description, price, discount, coupons, type,timetakes " +
		                       "FROM service WHERE serviceID = ?;";
		        
		        preparedStatement = connection.prepareStatement(query);
		        preparedStatement.setString(1, serviceID); // Set the type parameter
		        resultSet = preparedStatement.executeQuery();
		        ...
		        while (resultSet.next()) {
		            ...
		            aa.setCoupons(resultSet.getString("coupons"));
		            aa.setType(resultSet.getString("type"));
		            aa.setTimetakes(resultSet.getString("timetakes"));
		            allS.add(aa);
		        }
		    } catch (Exception e) {
		        // Handle exceptions
