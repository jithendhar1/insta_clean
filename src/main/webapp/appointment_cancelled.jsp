...
/* if (!whereClause.isEmpty()) {
    ...
    tax = AppointmentDAO.getFilteredAppointmentCancelled();
} else { */
...
<div class="row justify-content-center align-items-center" id = "flag1">
   ...
   <!-- Pagination links -->
    ...
    <% if (pageno > 1) { %>
        <a href="appointment_cancelled.jsp?page=<%=pageno - 1%>">Previous</a>
    <% } %>
    ...
    <% for (int i = 1; i <= noOfPages; i++) { %>
        <% if (i == pageno) { %>
            <%=i%>
        <% } else { %>
            <a href="appointment_cancelled.jsp?page=<%=i%>"><%=i%></a>
        <% } %>
    <% } %>

    <% if (pageno < noOfPages) { %>
        <a href="appointment_cancelled.jsp?page=<%=pageno + 1%>">Next</a>
    <% } %>
