...
if (!whereClause.isEmpty()) {
    ...
    tax = AppointmentDAO.getFilteredAppointmentCurrent(whereClause, start, limit);
} else {
    // Retrieve all data based on the limit
    tax = AppointmentDAO.getFilteredAppointmentCurrent("", start, limit);
}
...
<div class="row justify-content-center align-items-center" id = "flag1">
   ...
   <!-- Pagination links -->
    ...
    <% if (pageno > 1) { %>
        <a href="appointment_current.jsp?page=<%=pageno - 1%>">Previous</a>
    <% } %>
    ...
    <% for (int i = 1; i <= noOfPages; i++) { %>
        <% if (i == pageno) { %>
            <%=i%>
        <% } else { %>
            <a href="appointment_current.jsp?page=<%=i%>"><%=i%></a>
        <% } %>
    <% } %>

    <% if (pageno < noOfPages) { %>
        <a href="appointment_current.jsp?page=<%=pageno + 1%>">Next</a>
    <% } %>
